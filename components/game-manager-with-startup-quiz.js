...
import { StartupQuiz } from './startup-quiz.js';
...
export const GameManager = {
    ...
    template: `
      <div>
        <!-- *** NEW: Startup Quiz - Shows before game *** -->
        <StartupQuiz
            ref="startupQuizRef"
            @on-quiz-complete="on_startup_quiz_complete"
        />

        <UsernameBlocker @on-set-username="set_username" v-if="show_game_ui"></UsernameBlocker>
        <GameoverBlocker
            v-if="is_game_over && show_game_ui"
            :game_score="score.points"
            :score_board_position="score.position"
            :max_position_count="all_scores.length"
        />

        <!-- *** NEW: Math Quiz Component *** -->
        <MathQuiz
            ref="mathQuizRef"
            @on-quiz-correct="on_quiz_correct"
            @on-quiz-incorrect="on_quiz_incorrect"
            v-if="show_game_ui"
        />

        <div class="row d-flex justify-content-center align-items-center" v-if="show_game_ui">
          <h1 id="headline">Ball Dropping Game</h1>
        </div>
        <div class="row gx-4 d-flex justify-content-center align-items-center" v-if="show_game_ui">
          <div class="col-3">
        ...
        <div class="table table-bordered w-100" v-if="show_game_ui">
    ...
    setup() {
        ...
        const mathQuizRef = Vue.ref(null);
        const startupQuizRef = Vue.ref(null);
        const show_game_ui = Vue.ref(false);
        ...
        // *** NEW: Handle when quiz is needed (board cleared) ***
        function on_quiz_needed() {
            if (mathQuizRef.value) {
                mathQuizRef.value.show_quiz_modal();
            }
        }

        // *** NEW: Handle correct quiz answer ***
        function on_quiz_correct() {
            if (gameBoardRef.value && gameBoardRef.value.gameEngine) {
                gameBoardRef.value.gameEngine.on_quiz_correct();
            }
        }

        // *** NEW: Handle incorrect quiz answer ***
        function on_quiz_incorrect() {
            if (gameBoardRef.value && gameBoardRef.value.gameEngine) {
                gameBoardRef.value.gameEngine.on_quiz_incorrect();
            }
            // Trigger game over flow
            game_over();
        }

        // *** NEW: Handle startup quiz completion ***
        function on_startup_quiz_complete(result) {
            console.log('Startup quiz completed:', result);
            show_game_ui.value = true;
            // Load scores after startup quiz completes
            load_all_scores();
        }

        // Load scores immediately on mount, so the list (local or remote)
        // is populated before the first game-over, not just after it.
        // NOTE: Moved to on_startup_quiz_complete to show after startup quiz
        // load_all_scores();
